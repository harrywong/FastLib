...
//    if(absv == value) // Apparently this is not a valid check on all compilers
    ...
    else if (value == -1) // add by me
    {
      // special encode -1, for compliance with old SH Level2 message.
      // normally, it will be encode to 0xFF
      destination.putByte(0x7F);
      destination.putByte(0x7F);
      destination.putByte(0x7F);
      destination.putByte(0x7F);
      destination.putByte(0xFF);
    }
...
FieldInstruction::encodeUnsignedInteger(DataDestination & destination, WorkingBuffer & buffer, uint64 value)
{
  ...
  {
      if (value >= 0x0000000000000040ULL) // added by me
      {
        // special encode for uint which > 0x40 and < 0x80, for compliance with old SH Level2 message.
        // normally, it will add have an addtional 0x00 before stardard byte
          destination.putByte(0x00);
      }
